...
@pytest.mark.parametrize("pacs_009_iso_xml", [("incorrect_structure", "pacs_009")], indirect=True)
def test_pacs_009_incorrect_structure(pacs_009_iso_xml):

    # Send the ISO file and get the raw HTTP response
    response = api_calls.send_iso_file(pacs_009_iso_xml, return_raw_response=True)
